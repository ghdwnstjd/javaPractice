...
//   두 정수의 나눗셈을 해주는 메소드(몫과 나머지 둘 다)
   int[] divide(int num1, int num2){
      int[] results = null; //정수 배열 형식의 results 변수 null로 초기화
      
      if(num2 != 0) { //divide메소드의 두번째 정수값이 0이 아니라면
         results = new int[2]; // results 배열을 정수 두 칸짜리 배열로 생성한다.
         
         results[0] = num1 / num2;  //results 배열 첫 번째 값은 divide 메소드의 첫 번째 값 나누기 두 번째 값한 것의 몫
         results[1] = num1 % num2; //results 배열 두 번째 값은 위 값의 나머지 
      }
      return results; //results 배열을 리턴
   }
...
//   5개의 정수를 입력받은 후 원하는 번째 값을 구해주는 메소드
   int getValue(int[] nums, int wantedNumber){
      return nums[wantedNumber - 1]; //리턴값이 배열이 아닌, 배열에 속한 하나의 값이므로 int형이 된다.
   }
...
//      여기서부터 최대값, 최소값 사용 가능
   ...
   public static void main(String[] args) {
      MethodTask mt = new MethodTask(); //MethodTask 메소드의 mt객체를 생성(객체화)
