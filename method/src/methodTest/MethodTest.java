...
//	2. 일반 메소드의 소속을 알려준다. -> new 클래스
//		- static 메소드에서 사용된 일반 메소드는 Heap 영역에서 찾는다.
//		- 클래스 타입의 변수를 선언할 때 Heap 영역에 할당되므로, 
//		  일반 메소드도 해당 변수를 통해 접근하여 사용할 수 있다.
	
	public static void main(String[] args) {
		// multiple을 쓰기 위해서는 클래스의 변수를 통해 접근한다.
		MethodTest mt = new MethodTest();	// mt는 new 연산자로 들어왔으니 heap 메모리에 할당되기 때문에 multiple로 접근이 가능하다.
		int result = mt.multiple(3, 6);
